...
from server.config import get_bokeh_port, BOKEH_URI
...
class WebSocketProxy(WebSocketHandler):
    ...
    def open(self, *args, **kwargs):
        ...
        IOLoop.current().spawn_callback(self._connect_to_server, self.uri, protocols)
    ...
    def _on_message_callback(self, message):
        if message is not None:
            IOLoop.current().spawn_callback(
                self._send_to_client, message, not isinstance(message, str)
            )
        else:
            self.close()
    ...
    def on_message(self, message):
        if message is not None:
            IOLoop.current().spawn_callback(
                self._send_to_server, message, not isinstance(message, str)
            )
        else:
            self.close()
